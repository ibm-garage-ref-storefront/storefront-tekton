...
# In the current version the door is allready exposed
#sleep 7
...
echo "send of the request to the the customer.add api"
curl -s -X POST -i "http://$ROUTE/micro/customer/add" -H "Content-Type: application/json" -H "Authorization: Bearer ${jwt}" -d "{\"username\": \"${username}\", \"password\": \"${password}\", \"firstName\": \"${username}\", \"lastName\": \"bar\", \"email\": \"${username}@jedi-masters.org\"}"
